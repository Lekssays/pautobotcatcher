...
function deployChaincode() {
  ...
  peer lifecycle chaincode package $1.tar.gz --path ./chaincodes/$1cc --lang $2 --label ${1}_${VERSION}
  ...
  do
    setVariables $orgId
    peer lifecycle chaincode approveformyorg -o $ORDERER_ADDRESS --ordererTLSHostnameOverride $ORDERER_HOSTNAME --channelID $CHANNEL_NAME --name $1 --version $VERSION --package-id $CC_PACKAGE_ID --sequence $VERSION  --tls --cafile $ORDERER_CA --signature-policy "OR ('Org1MSP.member','Org2MSP.member', 'Org3MSP.member')"
  done

  echo "Check for $1 commit readiness..."
  peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name $1 --version $CCVERSION --sequence $VERSION  --tls --cafile $ORDERER_CA --signature-policy "OR ('Org1MSP.member','Org2MSP.member', 'Org3MSP.member')" --output json
  
  echo "Committing $1 chaincode definition to channel..."
  peer lifecycle chaincode commit -o $ORDERER_ADDRESS --ordererTLSHostnameOverride $ORDERER_HOSTNAME --channelID $CHANNEL_NAME --name $1 --version $VERSION --sequence $VERSION  --tls --cafile $ORDERER_CA $PEERS_CERTIFICATES --signature-policy "OR ('Org1MSP.member','Org2MSP.member', 'Org3MSP.member')"
...
if [ "${MODE}" == "down" ]; then
  ...
  deployChaincode "detection" "java" $VERSION
elif [ "${MODE}" == "invokeCC" ]; then
  invokeChaincode "peer" "golang"
  invokeChaincode "voting" "golang"
  ...
  networkUp
  createChannel
  deployChaincode "detection" "java" "1"
  invokeChaincode "detection"
else
  printHelp
  exit 1
fi
